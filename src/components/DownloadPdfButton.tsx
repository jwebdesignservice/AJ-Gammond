...
export default function DownloadPdfButton({ contentId, filename, fullWidth = false }: DownloadPdfButtonProps) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`flex items-center justify-center gap-2 text-sm font-medium disabled:opacity-50 bg-[#2563EB] hover:bg-[#1d4ed8] text-white px-4 py-2.5 rounded-xl transition-colors ${fullWidth ? 'w-full' : ''}`}
